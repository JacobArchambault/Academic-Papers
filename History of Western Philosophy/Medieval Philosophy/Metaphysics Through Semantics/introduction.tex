...
that properties are sets of tuples, 
...
%\footnote{Frege himself, for instance, accepted the classical interpretation of $\rightarrow$ for arithmetic while regarding it as inadequate to a general interpretation of the meaning of `if ... then`. Likewise, Tarski took the functional nature of reference in his semantics to be a replacement for, rather than a representation of, the perceived imprecision of natural language reference.}
What it does mean, however, 
is that a semantics touching on matters meant to model an understanding of being via structures such as quantification or the establishing of a domain, 
considered in itself and on its intended interpretation, 
cannot avoid decision on ontological questions. 
%Indeed, this is the foundation for the possibility of recognizing that an ontology can be inadequate, 
