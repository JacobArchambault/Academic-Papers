...
\section{The semantics of `metaphysics': expressibility and independence revisited}
In this way, 
...
such that, given a translation $f$ from $S$ into $S'$, a claim $X$ is provable in $S$ exactly when  $f(X)$ is in $S'$.
...
with the base case being that a claim $A$ holds classicallly if and only if $\neg\neg A$ holds intuitionistically. 
This particular example also shows that a language may be fundamentally more expressive than another even when we have the same set of lexical elements defined across both systems, and even when the weaker system appears at first glance to be the stronger one. 
%As G\"{o}del's incompleteness theorems show, it is possible for a framework to be too expressive, 
...
Now, broadening this point about semantic frameworks to `frameworks' in the broader sense of languages and traditions, 
we can say that Anglophone philosophy of the past seventy or so years as primarily interpreted the question of what there is one equivalent to that of what \emph{things} there \emph{are}. 
In other words, 
that core part of metaphysics that since Christian Wolff has been called `ontology' is taken as a discipline whose theorems are, 
in their basic syntax, 
positive or negated existential, non-copulative uses of the verb `is' joined to a common or proper noun, 
...
This understanding of ontology, however, is an attentuation of a much broader notion,
and arguably not the sense in which that part of metaphysics was understood by the ancient and medieval interpreters of Aristotle.
Ontology isn't concerned primarily and fundamentally with the question of what things there are, but of what \emph{being is}. 
Given this concern, one limited way of answering this question is in a universally quantified disjunction of the aforementioned kind. 
But equally much if not more is left out of this understanding: 
take, for instance, the propositions grounding the convertibility of transcendental predicates with being itself: to be is to be good, to be one, to be distinct.
...
but it is that even all these years after Aristotle, 
...
%Objections:
	...
	% 
%Goal: solve the problem of talking past each other by expanding the framework. 
