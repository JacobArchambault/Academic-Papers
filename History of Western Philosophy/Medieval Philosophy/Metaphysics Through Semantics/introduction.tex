...
`semantics' is said in four ways: 
Minimally, it can refer to a purely mathematical discipline providing rules for recursively mapping symbols of an artificial language via interpretation functions to domain elements, sets, types, truth functions, etc. - essentially what today we mean by model theory. 
...
% - Counterexample: properties are represented as sets, but very few actually accept this. 
...
% Using classical logic in limited cases does not thereby deny the existence of modality. 
...
%Distinguishing in in the sense of inhering from in in the sense of parthood
\subsection{The semantics of `metaphysics'}
Now, broadening this point about semantic frameworks to `frameworks' in a broader sense to include philosophical traditions working in natural language, 
we can say that Anglophone philosophy of the past seventy or so years has primarily interpreted the question of what there is as one equivalent to that of what \emph{things} there \emph{are}. 
In other words, 
%that core part of metaphysics that since Christian Wolff has been called 
ontology is taken as a discipline whose theorems are, 
...
%Both overgenerates (e.g. everything is or isn't my eldest daughter isn't a thesis of metaphysics.) and undergenerates (neglects transcendentals, senses of being)
Given this concern, 
one limited way of answering this question is in a universally quantified disjunction of the aforementioned kind. 
But much is left out of this understanding: 
...
%On the other hand, much is included that shouldn't be: e.g. claims like `there are baseballs' or `everything is or isn't my eldest daughter' are arguably not metaphysical claims at all. 
