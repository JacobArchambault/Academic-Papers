...
Since at least \autocite{Lejewski1954},
we've known that actualist and possibilist semantics are expressively equivalent, 
given a one-to-one translation schema from the one to the other on which, e.g. 
...
exactly when $\forall x (Ex \rightarrow Fx)$ and $\exists x(Ex \wedge Fx)$ hold, respectively, in possibilist semantics.\footnote{
	The full translation schema involves recursion, 
	and the explication here is intentionally cursory.
