...
This can occur in a trivial sense when a language introduces new syntactic elements, 
but such expressiveness may be merely apparent, 
...
whose semantics may be defined such that $A \rightarrow B$ is merely a shorthand for $\neg(A \wedge \neg B)$. 
Thus, adding the new symbol doesn't actually increase the expressiveness of the language except in the sense that allows for some complex expressions to be made slightly less verbose. 
In other cases, e.g. by augmenting classical first-order logic with an identity predicate, we can add a new logical constant to a language that thereby allows for teh expression of previously inexpressible theorems and entailments. 
